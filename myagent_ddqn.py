...
#             loss.backwards()
        ...
        target_act = self.qnetwork_local(next_states).detach().max(1)[1]
        
        temp = np.array(self.qnetwork_target(next_states).detach())
        #print(temp.size())
        temp = temp[np.arange(len(temp)), target_act]
        target_aval = torch.from_numpy(temp).float().unsqueeze(1).to(device)
